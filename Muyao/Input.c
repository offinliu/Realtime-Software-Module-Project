...
#include <math.h>
#include <stdlib.h>
#include <stddef.h> //for linux compatibility

const float pi = 3.14159265359;

typedef struct _listnode //main linkedlist node
{
...
typedef struct _linkedlist //main linkedlist
{
	int size;
	ListNode *head;
	ListNode *tail;
...
int extract(LinkedList *ll, LinkedListEx *ex, int t1, int selector);		  //int for error catching
...
{ //main() used to simulate input from main menu
	...
	printList(&mainlist);
	check = 0;
	check = extract(&mainlist, &extractedlist, 12, 2);
	if (check != 0)
		perror("insert fail.");
...
{ //ll is main linked list. t1 is type, 01 for 2D, 02 for 3D. selector is for d1---c2
	...
	while (mov != NULL)
	{
		if (((t1 / 10 == 0) && ((mov->type) % 10 == t1 % 10)) || (mov->type == t1))
		{
			...
			{
			case 1: //extract d1
				i = insertNodeEx(ex, mov->d1);
				break;
			case 2: //extract d2
				i = insertNodeEx(ex, mov->d2);
				break;
			case 3: //extract d3
				if (t1 % 10 != 2)
				{
				...
				i = insertNodeEx(ex, mov->d3);
				break;
			case 4: // extract c1
				i = insertNodeEx(ex, mov->c1);
				break;
			case 5: //extract c2
				i = insertNodeEx(ex, mov->c2);
				break;
			default:
